...
class TestStaleFailuresAreNotNews:
    """2026-08-09: phi alerted on crashes from July 2, 5, 13 and 21 in one
    morning, and said so herself — "stale, not fresh breaks".

    A run that crashed before starting has no end_time, and most do. The
    monitor sorted on END_TIME_DESC, so the 50-row window over an
    indefinitely-retained failure history was an unstable subset: old runs
    drifted in, and one that had never landed in the window before was
    indistinguishable from a fresh break. Absence from the seen-list is not
    evidence of newness; only the timestamp is.
    """
    ...
    def test_a_july_crash_is_not_reported_in_august(self):
        ...
        from bot.core.workflow_failures import fresh_failures

        now = time.time()
        runs = [
            self._run("phi-curation-e9a18699", 27 * 86400, now),
            self._run("bisk-snapshot-98e6926f", 19 * 86400, now),
            self._run("transform-cf59c0ee", 38 * 86400, now),
        ]
        assert fresh_failures(runs, now) == []

    def test_a_real_break_still_gets_through(self):
        import time

        from bot.core.workflow_failures import fresh_failures

        now = time.time()
        fresh = self._run("fastmcp-brief-e398ed98", 120, now)
        stale = self._run("phi-atlas-d593f4cf", 35 * 86400, now)
        assert fresh_failures([fresh, stale], now) == [fresh]

    def test_an_unreadable_timestamp_is_not_silenced(self):
        import time

        from bot.core.workflow_failures import fresh_failures

        runs = [{"id": "x", "name": "x", "state": {}}]
        assert fresh_failures(runs, time.time()) == runs
