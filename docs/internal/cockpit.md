...
| `/market` | phi's top chicken market book: current position (with round context), all-time p&l + net-worth sparkline, trade ledger with settlement outcomes, and her own posts about the trades. reads topchicken.cee.wtf via the bot's `/api/chicken/*` proxy (upstream has no CORS headers) + the bsky public API directly |
