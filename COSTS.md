...
> **Mandate.** This file tracks the infrastructure costs attributed to **bot** and
...
curl -s https://hub.waow.tech/api/costs.json | jq '{
  ...
  this_repo_monthly_usd: (
    [ .lineItems[] | select(.service as $s | ["zzstoatzz-phi"] | index($s)) ]
    | (map(.amount) | add // 0) / 100
  ),
  lines: [ .lineItems[] | select(.service as $s | ["zzstoatzz-phi"] | index($s))
...
Services attributed to this repo: `zzstoatzz-phi`. If that list is
...
- biggest line is usually `zzstoatzz-phi` — check its utilization and right-size before anything else.
